...
def db_add_shop_item(guild_id: int, name: str, price: int, image_url: str = None,
                     ...
                     show_duration: int = 1, requires_text: int = 0, text_label: str = None) -> int:
    conn = get_db()
    c = conn.execute(
        """INSERT INTO shop_items
           (guild_id, name, price, image_url, is_temporary, duration_days, show_duration, requires_text, text_label)
           VALUES (?,?,?,?,?,?,?,?,?)""",
        (guild_id, name, price, image_url, is_temporary, duration_days, show_duration, requires_text, text_label)
    )
    item_id = c.lastrowid
...
def db_assign_monthly_quests(guild_id: int, user_id: int, month_key: str):
    """Assign one random quest per rarity for this user/month (skip if already assigned)."""
...
async def restore_from_discord(bot: commands.Bot):
    if not os.path.exists(BACKUP_REGISTRY):
        print("[Restore] No backup registry — starting fresh.")
        return
    try:
        with open(BACKUP_REGISTRY, "r") as f:
...
async def update_streak_nickname(guild: discord.Guild, user_id: int, streak: int):
    ...
    try:
        ...
        pass  # Missing permissions — documented in README
    except Exception as e:
        print(f"[Streak] Nick update failed for {user_id}: {e}")
...
class ConfigXPMenu(_SubMenu):
    def build_embed(self, config: dict) -> discord.Embed:
        ...
        e.add_field(name="✅ Reaction Emoji",     value=config.get("reaction_emoji", "✅"),              inline=True)
        e.add_field(name="💎 Reaction XP",        value=f"**{config.get('reaction_xp', 50)} XP**",       inline=True)
    ...
    @discord.ui.button(label="Reaction XP",      style=discord.ButtonStyle.blurple, row=0)
    async def btn_react_xp(self, interaction, btn):
        ...
        async def submit(inter, value):
            ...
            await inter.response.send_message(f"✅ Reaction XP set to **{amt} XP**", ephemeral=True)
            await self._refresh(interaction)
        await interaction.response.send_modal(Modal1("Reaction XP", "XP awarded per manager reaction",
    ...
    async def btn_add(self, interaction: discord.Interaction, btn):
        async def submit(inter, v_name, v_price, v_image, v_temp, v_text):
            ...
            try:
                ...
                return
            image_url   = v_image.strip() or None
            ...
            item_id = db_add_shop_item(self.guild.id, v_name.strip(), price, image_url,
                                       is_temp, dur_days, 1, requires_text, text_label)
            tags = []
            if image_url: tags.append("🖼️ image")
            if is_temp:   tags.append(f"⏳ {days} days")
            if requires_text: tags.append(f"📝 requires text")
            await inter.response.send_message(
                f"✅ Added **{v_name.strip()}** — **{price} XP** (ID: `{item_id}`)\n" +
                ("  ".join(tags) if tags else "Permanent, no text required"),
                ephemeral=True
            )
            await self._refresh(interaction)
    ...
    def build_embed(self, config: dict) -> discord.Embed:
        ...
        for ach_def in ACHIEVEMENT_DEFS:
            ...
            for t in tiers[:5]:
                ...
                tier_lines.append(f"Tier {t['tier']}: **{t['threshold']}** → {role_str}{enabled_str}")
            e.add_field(
    ...
    async def btn_role(self, interaction: discord.Interaction, btn):
        async def submit(inter, v_ach, v_tier, v_role):
            ...
            if not ach:
                ...
                return
            try:
                tier = int(v_tier.strip())
                if tier < 0 or tier > 4: raise ValueError
            except ValueError:
                await inter.response.send_message("❌ Tier must be 0–4.", ephemeral=True)
                return
            ...
            await inter.response.send_message(
                f"✅ **{ach['name']}** Tier {tier} role set to {role_str}", ephemeral=True)
            await self._refresh(interaction)
    ...
    async def btn_threshold(self, interaction: discord.Interaction, btn):
        async def submit(inter, v_ach, v_tier, v_threshold):
            ...
            try:
                ...
                if tier < 0 or tier > 4 or threshold < 1: raise ValueError
            except ValueError:
                await inter.response.send_message("❌ Invalid tier (0–4) or threshold.", ephemeral=True)
                return
            db_ensure_achievement_config(self.guild.id)
            conn = get_db()
            conn.execute(
            ...
            conn.close()
            await inter.response.send_message(
                f"✅ **{ach['name']}** Tier {tier} threshold set to **{threshold}**", ephemeral=True)
            await self._refresh(interaction)
        await interaction.response.send_modal(Modal3(
...
async def _handle_share(message: discord.Message, config: dict):
    ...
    conn_tmp = get_db()
    row_tmp = conn_tmp.execute("SELECT share_xp FROM guild_config WHERE guild_id=?", (guild_id,)).fetchone()
    conn_tmp.close()
    share_xp_base = row_tmp["share_xp"] if row_tmp and "share_xp" in row_tmp.keys() else 100
...
async def cmd_config(interaction: discord.Interaction):
    ...
    embed = config_status_embed(interaction.guild, config)
